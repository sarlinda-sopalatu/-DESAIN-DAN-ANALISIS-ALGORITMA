...
public class KnapsackLaboratorium {
    ...
    public static void main(String[] args) {
        // 1. Data Inventori Laboratorium
        String[] names = {
            "Mikroskop", "Tabung reaksi", "Centrifuge", "Pipet", 
            "Spectrophotometer", "Hot plate", "Gelas ukur", 
            "Incubator", "pH meter", "Autoclave"
        };
        int[] w = {15, 2, 8, 1, 12, 6, 3, 20, 4, 25}; // Berat/Volume
        int[] p = {120, 15, 85, 10, 150, 65, 25, 180, 55, 200}; // Keuntungan/Profit
        int M = 20; // Kapasitas Rak yang ditentukan
        int n = names.length;

        // 2. Matriks DP untuk menyimpan hasil sub-masalah
        int[][] dp = new int[n + 1][M + 1];

        // 3. Proses Pengisian Tabel (Tabulation)
        for (int i = 1; i <= n; i++) {
            for (int j = 0; j <= M; j++) {
                if (w[i - 1] <= j) {
                    // Pilihan: Ambil item atau lewati
        ...
        // 4. Melacak Item yang Terpilih (Traceback)
        int res = dp[n][M];
        int tempW = M;
